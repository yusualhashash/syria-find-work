...
import { createBlacksmith, updateBlacksmith } from "@/lib/blacksmiths/blacksmiths-actions"
...
export default function BlacksmithForm({ blacksmith }: { blacksmith?: Blacksmith }) {
    const router = useRouter()
    const [loading, setLoading] = useState(false)
    const [selectedCity, setSelectedCity] = useState(blacksmith?.city || "")
    const [selectedGender, setSelectedGender] = useState(blacksmith?.gender || "")
    const [whatsappNumberValue, setWhatsappNumberValue] = useState<string | undefined>("")
    const [error, setError] = useState<string>("")

    useEffect(() => {
        if (blacksmith?.whatsapp_number) {
            try {
                const phone = parsePhoneNumber(blacksmith.whatsapp_number)
                setWhatsappNumberValue(phone?.number)
            } catch {
                setWhatsappNumberValue(blacksmith.whatsapp_number)
            }
        }
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (blacksmith) {
                await updateBlacksmith(blacksmith.id, formData)
            } else {
                await createBlacksmith(formData)
            }

            router.push("/admin/blacksmiths")
            router.refresh()
        } catch (error: any) {
            console.error("Error saving blacksmith:", error)
            ...
            <CardContent className="p-2 sm:p-4">
                <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
                    ...
                    <div className="space-y-2">
                        ...
                        <div className={`${baseInputStyles} ${focusWithinStyles}`}>
                            <Input
                                ...
                                name="name"
                                type="text"
                                defaultValue={blacksmith?.name}
                                placeholder="أدخل اسم الحداد"
                                required
                    ...
                    <div className="space-y-2">
                        ...
                        <div className={`${baseInputStyles} ${focusWithinStyles}`}>
                            <Input
                                ...
                                name="surname"
                                type="text"
                                defaultValue={blacksmith?.surname}
                                placeholder="أدخل كنية الحداد"
                                required
                                className="w-full h-full text-right bg-transparent px-3 outline-none border-none focus:ring-0 focus:ring-offset-0"
                            />
                        </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <div className={`${textareaGroupStyles} ${focusWithinStyles}`}>
                            <InputGroupTextarea
                                id="address"
                                name="address"
                                defaultValue={blacksmith?.address || ""}
                                placeholder="أدخل عنوان الحداد"
                                required
                                rows={4}
                    ...
                    <div className="space-y-2">
                        ...
                        <div className={`${baseInputStyles} ${focusWithinStyles}`}>
                            <Input
                                ...
                                type="number"
                                min="0"
                                defaultValue={blacksmith?.experience_years}
                    ...
                    <div className="flex gap-3 pt-2">
                        ...
                        >
                            {loading ? "جاري الحفظ..." : blacksmith ? "تحديث الحداد" : "إضافة الحداد"}
                        </Button>
                        <Button
                            type="button"
                            onClick={() => router.back()}
