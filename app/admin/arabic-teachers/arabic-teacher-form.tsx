...
import { createArabicTeacher, updateArabicTeacher } from "@/lib/arabic-teachers/arabic-teachers-actions"
...
export default function ArabicTeacherForm({ arabicTeacher }: { arabicTeacher?: ArabicTeacher }) {
  const router = useRouter()
  const [loading, setLoading] = useState(false)
  const [selectedCity, setSelectedCity] = useState(arabicTeacher?.city || "")
  const [selectedGender, setSelectedGender] = useState(arabicTeacher?.gender || "")
  const [whatsappNumberValue, setWhatsappNumberValue] = useState<string | undefined>("")
  const [error, setError] = useState<string>("")

  useEffect(() => {
    if (arabicTeacher?.whatsapp_number) {
      try {
        const phone = parsePhoneNumber(arabicTeacher.whatsapp_number)
        setWhatsappNumberValue(phone?.number)
      } catch {
        setWhatsappNumberValue(arabicTeacher.whatsapp_number)
      }
    }
  }, [arabicTeacher?.whatsapp_number])
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (arabicTeacher) {
        await updateArabicTeacher(arabicTeacher.id, formData)
      } else {
        await createArabicTeacher(formData)
      }

      router.push("/admin/arabic-teachers")
      router.refresh()
    } catch (error: any) {
      console.error("Error saving Arabic teacher:", error)
      ...
      <CardContent className="p-2 sm:p-4">
        <form onSubmit={handleSubmit} className="space-y-2 sm:space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className={`${baseInputStyles} ${focusWithinStyles}`}>
              <Input
                ...
                name="name"
                type="text"
                defaultValue={arabicTeacher?.name}
          ...
          <div className="space-y-2">
            ...
            <div className={`${baseInputStyles} ${focusWithinStyles}`}>
              <Input
                ...
                name="surname"
                type="text"
                defaultValue={arabicTeacher?.surname}
          ...
          <div className="space-y-2">
            ...
            <div className={`${baseInputStyles} ${focusWithinStyles}`}>
              <Input
                ...
                min="1"
                max="99"
                defaultValue={arabicTeacher?.age}
          ...
          <div className="space-y-2">
            ...
            <div className={`${textareaGroupStyles} ${focusWithinStyles}`}>
              <InputGroupTextarea
                id="address"
                name="address"
                defaultValue={arabicTeacher?.address || ""}
          ...
          <div className="space-y-2">
            ...
            <div className={`${baseInputStyles} ${focusWithinStyles}`}>
              <Input
                ...
                type="number"
                min="0"
                defaultValue={arabicTeacher?.experience_years}
          ...
          <div className="space-y-2">
            ...
            <div className={`${textareaGroupStyles} ${focusWithinStyles}`}>
              <InputGroupTextarea
                id="notes"
                name="notes"
                defaultValue={arabicTeacher?.notes || ""}
          ...
          <div className="flex gap-3 pt-2">
            ...
            >
              {loading ? "جاري الحفظ..." : arabicTeacher ? "تحديث المدرس" : "إضافة المدرس"}
            </Button>
            <Button
              type="button"
              onClick={() => router.back()}
