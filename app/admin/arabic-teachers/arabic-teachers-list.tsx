...
import { getArabicTeachers, deleteArabicTeacher } from "@/lib/arabic-teachers/arabic-teachers-actions"
...
export default function ArabicTeachersList() {
  const [arabicTeachers, setArabicTeachers] = useState<ArabicTeacher[]>([])
  ...
  const loadArabicTeachers = async () => {
    try {
      const data = await getArabicTeachers()
      setArabicTeachers(data)
    } catch (error) {
      console.error("Error loading Arabic teachers:", error)
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleDelete = async (id: string) => {
    try {
      await deleteArabicTeacher(id)
      setArabicTeachers(arabicTeachers.filter((t) => t.id !== id))
    } catch (error) {
      console.error("Error deleting Arabic teacher:", error)
  ...
  if (arabicTeachers.length === 0) {
    return <div className="text-center py-12 text-gray-600">لا يوجد مدرسين لغة عربية مضافين بعد</div>
  }
          ...
          <CardContent className="p-4 sm:p-6 space-y-4">
            ...
            <div className="flex gap-3 pt-2">
              ...
              <Button variant="outline" className="flex-1 gap-2 text-sm sm:text-base bg-white text-gray-900 border border-gray-300 hover:bg-gray-100" asChild>
                <Link href={`/admin/arabic-teachers/edit/${teacher.id}`}>
