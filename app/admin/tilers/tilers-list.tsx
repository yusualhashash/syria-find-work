...
import { getTilers, deleteTiler } from "@/lib/tilers/tilers-actions"
...
export default function TilersList() {
    const [tilers, setTilers] = useState<Tiler[]>([])
    ...
    const handleDelete = async (id: string) => {
        try {
            await deleteTiler(id)
            setTilers(tilers.filter((t) => t.id !== id))
        } catch (error) {
            console.error("Error deleting tiler:", error)
            alert("حدث خطأ أثناء حذف المبلط")
        }
    }
    ...
    if (tilers.length === 0) {
        return <div className="text-center py-12 text-gray-600">لا يوجد مبلطين مضافين بعد</div>
    }

    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {tilers.map((tiler) => (
                <Card key={tiler.id} className="overflow-hidden rounded-lg shadow-md bg-white text-gray-900">
                    <CardContent className="p-4 sm:p-6 space-y-4">
                        <div className="flex items-center justify-end gap-4 mb-4">
                            <div className="flex flex-col text-right grow">
                                <h3 className="text-xl sm:text-2xl font-bold">{tiler.name} {tiler.surname}</h3>
                                <div className="flex items-center flex-row-reverse justify-end gap-2 text-gray-600 text-sm mt-1">
                                    <span>{tiler.gender === "male" ? "ذكر" : "أنثى"}</span>
                                    <User className="w-4 h-4" />
                                    <span>{tiler.age} سنة</span>
                                    <Cake className="w-4 h-4" />
                                </div>
                                <Badge variant="secondary" className="mt-2 w-fit gap-1 text-xs bg-gray-200 text-gray-800 flex-row-reverse">
                                    <span>خبرة {tiler.experience_years} سنوات</span>
                        ...
                        <div className="space-y-2 text-sm sm:text-base text-gray-700 text-right">
                            {tiler.city && (
                                <div className="flex items-center flex-row-reverse justify-end gap-2">
                                    <span>{tiler.city}</span>
                                    <MapPin className="w-4 h-4" />
                                </div>
                            )}
                            {tiler.address && (
                                <div className="flex items-center flex-row-reverse justify-end gap-2">
                                    <span>{tiler.address}</span>
                                    <Home className="w-4 h-4" />
                                </div>
                            )}
                            {tiler.whatsapp_number && (
                                <div className="flex items-center flex-row-reverse justify-end gap-2">
                                    <span dir="ltr">{tiler.whatsapp_number}</span>
                                    <Phone className="w-4 h-4" />
                                </div>
                            )}
                            {tiler.notes && (
                                <div className="flex items-center flex-row-reverse justify-end gap-2">
                                    <p className="text-gray-600">{tiler.notes}</p>
                        ...
                        <div className="flex gap-3 pt-2">
                            <AlertDialog>
                                ...
                                <AlertDialogContent dir="rtl" className="bg-white">
                                    <AlertDialogHeader>
                                        ...
                                        <AlertDialogDescription>
                                            سيتم حذف المبلط "{tiler.name} {tiler.surname}" نهائياً ولا يمكن التراجع عن هذا الإجراء.
                                        </AlertDialogDescription>
                                    </AlertDialogHeader>
                                    <AlertDialogFooter className="flex flex-row gap-3 justify-end">
                                        <AlertDialogAction
                                            onClick={() => handleDelete(tiler.id)}
                            ...
                            <Button variant="outline" className="flex-1 gap-2 text-sm sm:text-base bg-white text-gray-900 border border-gray-300 hover:bg-gray-100" asChild>
                                <Link href={`/admin/tilers/edit/${tiler.id}`}>
