...
import EnglishTeacherForm from "../../english-teacher-form"

export async function generateStaticParams() {
    return [{ id: "static-placeholder-id" }]
}

export default async function EditEnglishTeacherPage({
    params,
}: {
    ...
    const { data: englishTeacher, error } = await supabase.from("english_teachers").select("*").eq("id", id).single()

    if (error || !englishTeacher) {
        redirect("/admin/english-teachers")
    }
        ...
        <div dir="rtl" className="min-h-screen bg-linear-to-br from-indigo-50 to-purple-100">
            <div className="max-w-4xl mx-auto p-4 sm:p-6">
                <div className="bg-white rounded-2xl shadow-xl p-4 sm:p-8">
                    ...
                    <EnglishTeacherForm englishTeacher={englishTeacher} />
                </div>
            </div>
        </div>
    )
