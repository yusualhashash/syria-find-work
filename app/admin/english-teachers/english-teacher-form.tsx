...
import { createEnglishTeacher, updateEnglishTeacher } from "@/lib/english-techers/english-teachers-actions"
import PhoneInput, { isValidPhoneNumber, parsePhoneNumber } from "react-phone-number-input" // Import PhoneInput
import ar from "react-phone-number-input/locale/ar" // Import Arabic locale
...
export default function EnglishTeacherForm({ englishTeacher }: { englishTeacher?: EnglishTeacher }) {
    const router = useRouter()
    const [loading, setLoading] = useState(false)
    const [selectedCity, setSelectedCity] = useState(englishTeacher?.city || "")
    const [selectedGender, setSelectedGender] = useState(englishTeacher?.gender || "")
    const [whatsappNumberValue, setWhatsappNumberValue] = useState<string | undefined>("") // New state for phone input
    const [error, setError] = useState<string>("") // New state for error messages

    useEffect(() => {
        if (englishTeacher?.whatsapp_number) {
            try {
                const phone = parsePhoneNumber(englishTeacher.whatsapp_number)
                setWhatsappNumberValue(phone?.number)
            } catch {
                setWhatsappNumberValue(englishTeacher.whatsapp_number)
            }
        }
    }, [englishTeacher?.whatsapp_number])
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            formData.set("whatsapp_number", whatsappNumberValue || "") // Set formatted phone number

            if (englishTeacher) {
                await updateEnglishTeacher(englishTeacher.id, formData)
            } else {
                await createEnglishTeacher(formData)
            }

            router.push("/admin/english-teachers")
            router.refresh()
        } catch (error: any) {
            console.error("Error saving English teacher:", error)
            ...
            <CardContent className="p-2 sm:p-4">
                <form onSubmit={handleSubmit} className="space-y-2 sm:space-y-4">
                    ...
                    <div className="space-y-2">
                        ...
                        <div className={`${baseInputStyles} ${focusWithinStyles}`}>
                            <Input
                                ...
                                name="name"
                                type="text"
                                defaultValue={englishTeacher?.name}
                    ...
                    <div className="space-y-2">
                        ...
                        <div className={`${baseInputStyles} ${focusWithinStyles}`}>
                            <Input
                                ...
                                name="surname"
                                type="text"
                                defaultValue={englishTeacher?.surname}
                                placeholder="أدخل كنية المدرس"
                                required
                                className="w-full h-full text-right bg-transparent px-3 outline-none border-none focus:ring-0 focus:ring-offset-0"
                            />
                        </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <div className={`${baseInputStyles} ${focusWithinStyles}`}>
                            <Input
                                ...
                                min="1"
                                max="99"
                                defaultValue={englishTeacher?.age}
                    ...
                    <div className="space-y-2">
                        ...
                        <div className={`${textareaGroupStyles} ${focusWithinStyles}`}>
                            <InputGroupTextarea
                                id="address"
                                name="address"
                                defaultValue={englishTeacher?.address || ""}
                                placeholder="أدخل عنوان المدرس"
                                required
                                rows={4}
                    ...
                    <div className="space-y-2">
                        ...
                        <div className={`${baseInputStyles} ${focusWithinStyles}`}>
                            <Input
                                ...
                                type="number"
                                min="0"
                                defaultValue={englishTeacher?.experience_years}
                    ...
                    <div className="space-y-2">
                        ...
                        <div className={`${textareaGroupStyles} ${focusWithinStyles}`}>
                            <InputGroupTextarea
                                id="notes"
                                name="notes"
                                defaultValue={englishTeacher?.notes || ""}
                    ...
                    <div className="flex gap-3 pt-2">
                        ...
                        >
                            {loading ? "جاري الحفظ..." : englishTeacher ? "تحديث المدرس" : "إضافة المدرس"}
                        </Button>
                        <Button
                            type="button"
                            onClick={() => router.back()}
