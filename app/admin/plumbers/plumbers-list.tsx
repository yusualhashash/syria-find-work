...
import { getPlumbers, deletePlumber } from "@/lib/plumbers/plumbers-actions"
...
export default function PlumbersList() {
    ...
    const loadPlumbers = async () => {
        try {
            ...
            console.error("Error loading Plumbers:", error)
        } finally {
            setLoading(false)
        }
    }
    ...
    const handleDelete = async (id: string) => {
        try {
            await deletePlumber(id)
            setPlumbers(plumbers.filter((p) => p.id !== id))
        } catch (error) {
    ...
    if (plumbers.length === 0) {
        return <div className="text-center py-12 text-gray-600">لا يوجد معلمين تمديدات صحية مضافين بعد</div>
    }

    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {plumbers.map((plumber) => (
                <Card key={plumber.id} className="overflow-hidden rounded-lg shadow-md bg-white text-gray-900">
                    <CardContent className="p-4 sm:p-6 space-y-4">
                        <div className="flex items-center justify-end gap-4 mb-4">
                            <div className="flex flex-col text-right grow">
                                <h3 className="text-xl sm:text-2xl font-bold">{plumber.name} {plumber.surname}</h3>
                                <div className="flex items-center flex-row-reverse justify-end gap-2 text-gray-600 text-sm mt-1">
                                    <span>{plumber.gender === "male" ? "ذكر" : "أنثى"}</span>
                                    <User className="w-4 h-4" />
                                    <span>{plumber.age} سنة</span>
                                    <Cake className="w-4 h-4" />
                                </div>
                                <Badge variant="secondary" className="mt-2 w-fit gap-1 text-xs bg-gray-200 text-gray-800 flex-row-reverse">
                                    <span>خبرة {plumber.experience_years} سنوات</span>
                        ...
                        <div className="space-y-2 text-sm sm:text-base text-gray-700 text-right">
                            ...
                            {plumber.whatsapp_number && (
                                <div className="flex items-center flex-row-reverse justify-end gap-2">
                                    <span dir="ltr">{plumber.whatsapp_number}</span>
                                    <Phone className="w-4 h-4" />
                                </div>
                            )}
                            {plumber.notes && (
                                <div className="flex items-center flex-row-reverse justify-end gap-2">
                                    <p className="text-gray-600">{plumber.notes}</p>
                        ...
                        <div className="flex gap-3 pt-2">
                            <AlertDialog>
                                ...
                                <AlertDialogContent dir="rtl" className="bg-white">
                                    <AlertDialogHeader>
                                        ...
                                        <AlertDialogDescription>
                                            سيتم حذف المعلم "{plumber.name} {plumber.surname}" نهائياً ولا يمكن التراجع عن هذا الإجراء.
                                        </AlertDialogDescription>
                                    </AlertDialogHeader>
                            ...
                            <Button variant="outline" className="flex-1 gap-2 text-sm sm:text-base bg-white text-gray-900 border border-gray-300 hover:bg-gray-100" asChild>
                                <Link href={`/admin/plumbers/edit/${plumber.id}`}>
