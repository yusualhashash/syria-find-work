...
import { createPlumber, updatePlumber } from "@/lib/plumbers/plumbers-actions"
...
export default function PlumberForm({ plumber }: { plumber?: Plumber }) {
    const router = useRouter()
    const [loading, setLoading] = useState(false)
    const [selectedCity, setSelectedCity] = useState(plumber?.city || "")
    const [selectedGender, setSelectedGender] = useState(plumber?.gender || "")
    const [whatsappNumberValue, setWhatsappNumberValue] = useState<string | undefined>("")
    const [error, setError] = useState<string>("")

    useEffect(() => {
        if (plumber?.whatsapp_number) {
            try {
                const phone = parsePhoneNumber(plumber.whatsapp_number)
                setWhatsappNumberValue(phone?.number)
            } catch {
                setWhatsappNumberValue(plumber.whatsapp_number)
            }
        }
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (plumber) {
                await updatePlumber(plumber.id, formData)
            } else {
                await createPlumber(formData)
            }

            router.push("/admin/plumbers")
            router.refresh()
        } catch (error: any) {
            ...
            <CardContent className="p-2 sm:p-4">
                <form onSubmit={handleSubmit} className="space-y-2 sm:space-y-4">
                    ...
                    <div className="space-y-2">
                        ...
                        <div className={`${baseInputStyles} ${focusWithinStyles}`}>
                            <Input
                                ...
                                type="number"
                                min="0"
                                defaultValue={plumber?.experience_years}
                    ...
                    <div className="flex gap-3 pt-2">
                        ...
                        >
                            {loading ? "جاري الحفظ..." : plumber ? "تحديث المعلم" : "إضافة المعلم"}
                        </Button>
                        <Button
                            type="button"
                            onClick={() => router.back()}
