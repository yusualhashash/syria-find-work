...
import { getElectricianTeachers, deleteElectricianTeacher } from "@/lib/electrician-teachers/electrician-teachers-actions"
...
export default function ElectricianTeachersList() {
    const [electricianTeachers, setElectricianTeachers] = useState<ElectricianTeacher[]>([])
    const [loading, setLoading] = useState(true)

    useEffect(() => {
        loadElectricianTeachers()
    }, [])

    const loadElectricianTeachers = async () => {
        try {
            const data = await getElectricianTeachers()
            setElectricianTeachers(data)
        } catch (error) {
            console.error("Error loading Electrician teachers:", error)
        } finally {
            setLoading(false)
        }
    }
    ...
    const handleDelete = async (id: string) => {
        try {
            await deleteElectricianTeacher(id)
            setElectricianTeachers(electricianTeachers.filter((t) => t.id !== id))
        } catch (error) {
            console.error("Error deleting Electrician teacher:", error)
    ...
    if (electricianTeachers.length === 0) {
        return <div className="text-center py-12 text-gray-600">لا يوجد معلمين كهرباء مضافين بعد</div>
    }

    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {electricianTeachers.map((teacher) => (
                    ...
                    <CardContent className="p-4 sm:p-6 space-y-4">
                        ...
                        <div className="flex gap-3 pt-2">
                            <AlertDialog>
                                ...
                                <AlertDialogContent dir="rtl" className="bg-white">
                                    <AlertDialogHeader>
                                        ...
                                        <AlertDialogDescription>
                                            سيتم حذف المعلم "{teacher.name} {teacher.surname}" نهائياً ولا يمكن التراجع عن هذا الإجراء.
                                        </AlertDialogDescription>
                                    </AlertDialogHeader>
                            ...
                            <Button variant="outline" className="flex-1 gap-2 text-sm sm:text-base bg-white text-gray-900 border border-gray-300 hover:bg-gray-100" asChild>
                                <Link href={`/admin/electrician-teachers/edit/${teacher.id}`}>
