...
import { createElectricianTeacher, updateElectricianTeacher } from "@/lib/electrician-teachers/electrician-teachers-actions"
import PhoneInput, { isValidPhoneNumber, parsePhoneNumber } from "react-phone-number-input"
import ar from "react-phone-number-input/locale/ar"
import "react-phone-number-input/style.css"
import { Alert, AlertDescription } from "@/components/ui/alert"

type ElectricianTeacher = {
    id: string
    name: string
    surname: string
...
export default function ElectricianTeacherForm({ electricianTeacher }: { electricianTeacher?: ElectricianTeacher }) {
    const router = useRouter()
    const [loading, setLoading] = useState(false)
    const [selectedCity, setSelectedCity] = useState(electricianTeacher?.city || "")
    const [selectedGender, setSelectedGender] = useState(electricianTeacher?.gender || "")
    const [whatsappNumberValue, setWhatsappNumberValue] = useState<string | undefined>("")
    const [error, setError] = useState<string>("")

    useEffect(() => {
        if (electricianTeacher?.whatsapp_number) {
            try {
                const phone = parsePhoneNumber(electricianTeacher.whatsapp_number)
                setWhatsappNumberValue(phone?.number)
            } catch {
                setWhatsappNumberValue(electricianTeacher.whatsapp_number)
            }
        }
    }, [electricianTeacher?.whatsapp_number])
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (electricianTeacher) {
                await updateElectricianTeacher(electricianTeacher.id, formData)
            } else {
                await createElectricianTeacher(formData)
            }

            router.push("/admin/electrician-teachers")
            router.refresh()
        } catch (error: any) {
            console.error("Error saving Electrician teacher:", error)
            ...
            <CardContent className="p-2 sm:p-4">
                <form onSubmit={handleSubmit} className="space-y-2 sm:space-y-4">
                    ...
                    <div className="space-y-2">
                        ...
                        <div className={`${baseInputStyles} ${focusWithinStyles}`}>
                            <Input
                                ...
                                name="name"
                                type="text"
                                defaultValue={electricianTeacher?.name}
                    ...
                    <div className="space-y-2">
                        ...
                        <div className={`${baseInputStyles} ${focusWithinStyles}`}>
                            <Input
                                ...
                                name="surname"
                                type="text"
                                defaultValue={electricianTeacher?.surname}
                                placeholder="أدخل كنية المعلم"
                                required
                                className="w-full h-full text-right bg-transparent px-3 outline-none border-none focus:ring-0 focus:ring-offset-0"
                            />
                        </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <div className={`${baseInputStyles} ${focusWithinStyles}`}>
                            <Input
                                ...
                                min="1"
                                max="99"
                                defaultValue={electricianTeacher?.age}
                    ...
                    <div className="space-y-2">
                        ...
                        <div className={`${textareaGroupStyles} ${focusWithinStyles}`}>
                            <InputGroupTextarea
                                id="address"
                                name="address"
                                defaultValue={electricianTeacher?.address || ""}
                    ...
                    <div className="space-y-2">
                        ...
                        <div className={`${baseInputStyles} ${focusWithinStyles}`}>
                            <Input
                                ...
                                type="number"
                                min="0"
                                defaultValue={electricianTeacher?.experience_years}
                    ...
                    <div className="space-y-2">
                        ...
                        <div className={`${textareaGroupStyles} ${focusWithinStyles}`}>
                            <InputGroupTextarea
                                id="notes"
                                name="notes"
                                defaultValue={electricianTeacher?.notes || ""}
                    ...
                    <div className="flex gap-3 pt-2">
                        ...
                        >
                            {loading ? "جاري الحفظ..." : electricianTeacher ? "تحديث المعلم" : "إضافة المعلم"}
                        </Button>
                        <Button
                            type="button"
                            onClick={() => router.back()}
