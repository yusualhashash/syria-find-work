...
import { createUnskilledWorker, updateUnskilledWorker } from "@/lib/unskilled-workers/unskilled-workers-actions"
...
export default function UnskilledWorkerForm({ unskilledWorker }: { unskilledWorker?: UnskilledWorker }) {
    const router = useRouter()
    const [loading, setLoading] = useState(false)
    const [selectedCity, setSelectedCity] = useState(unskilledWorker?.city || "")
    const [selectedGender, setSelectedGender] = useState(unskilledWorker?.gender || "")
    const [whatsappNumberValue, setWhatsappNumberValue] = useState<string | undefined>("")
    const [error, setError] = useState<string>("")

    useEffect(() => {
        if (unskilledWorker?.whatsapp_number) {
            try {
                const phone = parsePhoneNumber(unskilledWorker.whatsapp_number)
                setWhatsappNumberValue(phone?.number)
            } catch {
                setWhatsappNumberValue(unskilledWorker.whatsapp_number)
            }
        }
    }, [unskilledWorker?.whatsapp_number])
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const data = {
                ...
                experience_years: parseInt(String(formData.get("experience_years") || "0")) || 0, // Ensure a default of 0 if empty or NaN
                notes: formData.get("notes") as string | null,
            }

            if (unskilledWorker) {
                await updateUnskilledWorker(unskilledWorker.id, data)
            } else {
                await createUnskilledWorker(data)
            }

            router.push("/admin/unskilled-workers")
            router.refresh()
        } catch (error: any) {
            console.error("Error saving unskilled worker:", error)
            ...
            <CardContent className="p-2 sm:p-4">
                <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
                    ...
                    <div className="space-y-2">
                        ...
                        <div className={`${baseInputStyles} ${focusWithinStyles}`}>
                            <Input
                                ...
                                name="name"
                                type="text"
                                defaultValue={unskilledWorker?.name}
                    ...
                    <div className="space-y-2">
                        ...
                        <div className={`${baseInputStyles} ${focusWithinStyles}`}>
                            <Input
                                ...
                                name="surname"
                                type="text"
                                defaultValue={unskilledWorker?.surname}
                                placeholder="أدخل كنية العامل"
                                required
                                className="w-full h-full text-right bg-transparent px-3 outline-none border-none focus:ring-0 focus:ring-offset-0"
                            />
                        </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <div className={`${baseInputStyles} ${focusWithinStyles}`}>
                            <Input
                                ...
                                min="1"
                                max="99"
                                defaultValue={unskilledWorker?.age}
                    ...
                    <div className="space-y-2">
                        ...
                        <div className={`${textareaGroupStyles} ${focusWithinStyles}`}>
                            <InputGroupTextarea
                                id="address"
                                name="address"
                                defaultValue={unskilledWorker?.address || ""}
                    ...
                    <div className="space-y-2">
                        ...
                        <div className={`${baseInputStyles} ${focusWithinStyles}`}>
                            <Input
                                ...
                                type="number"
                                min="0"
                                defaultValue={unskilledWorker?.experience_years}
                    ...
                    <div className="space-y-2">
                        ...
                        <div className={`${textareaGroupStyles} ${focusWithinStyles}`}>
                            <InputGroupTextarea
                                id="notes"
                                name="notes"
                                defaultValue={unskilledWorker?.notes || ""}
                    ...
                    <div className="flex gap-3 pt-2">
                        ...
                        >
                            {loading ? "جاري الحفظ..." : unskilledWorker ? "تحديث العامل" : "إضافة العامل"}
                        </Button>
                        <Button
                            type="button"
                            onClick={() => router.back()}
