...
import { createCarRepairman, updateCarRepairman } from "@/lib/car-repairmen/car-repairmen-actions"
...
export default function CarRepairmanForm({ carRepairman }: { carRepairman?: CarRepairman }) {
    const router = useRouter()
    const [loading, setLoading] = useState(false)
    const [selectedCity, setSelectedCity] = useState(carRepairman?.city || "")
    const [selectedGender, setSelectedGender] = useState(carRepairman?.gender || "")
    const [whatsappNumberValue, setWhatsappNumberValue] = useState<string | undefined>("")
    const [error, setError] = useState<string>("")

    useEffect(() => {
        if (carRepairman?.whatsapp_number) {
            try {
                const phone = parsePhoneNumber(carRepairman.whatsapp_number)
                setWhatsappNumberValue(phone?.number)
            } catch {
                setWhatsappNumberValue(carRepairman.whatsapp_number)
            }
        }
    }, [carRepairman?.whatsapp_number])
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (carRepairman) {
                await updateCarRepairman(carRepairman.id, formData)
            } else {
                await createCarRepairman(formData)
            }

            router.push("/admin/car-repairmen")
            router.refresh()
        } catch (error: any) {
            console.error("Error saving Car repairman:", error)
            ...
            <CardContent className="p-2 sm:p-4">
                <form onSubmit={handleSubmit} className="space-y-2 sm:space-y-4">
                    ...
                    <div className="space-y-2">
                        ...
                        <div className={`${baseInputStyles} ${focusWithinStyles}`}>
                            <Input
                                ...
                                name="name"
                                type="text"
                                defaultValue={carRepairman?.name}
                                placeholder="أدخل اسم المصلح"
                                required
                    ...
                    <div className="space-y-2">
                        ...
                        <div className={`${baseInputStyles} ${focusWithinStyles}`}>
                            <Input
                                ...
                                name="surname"
                                type="text"
                                defaultValue={carRepairman?.surname}
                                placeholder="أدخل كنية المصلح"
                                required
                                className="w-full h-full text-right bg-transparent px-3 outline-none border-none focus:ring-0 focus:ring-offset-0"
                            />
                        </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <div className={`${textareaGroupStyles} ${focusWithinStyles}`}>
                            <InputGroupTextarea
                                id="address"
                                name="address"
                                defaultValue={carRepairman?.address || ""}
                                placeholder="أدخل عنوان المصلح"
                                required
                                rows={4}
                    ...
                    <div className="space-y-2">
                        ...
                        <div className={`${baseInputStyles} ${focusWithinStyles}`}>
                            <Input
                                ...
                                type="number"
                                min="0"
                                defaultValue={carRepairman?.experience_years}
                    ...
                    <div className="space-y-2">
                        ...
                        <div className={`${textareaGroupStyles} ${focusWithinStyles}`}>
                            <InputGroupTextarea
                                id="notes"
                                name="notes"
                                defaultValue={carRepairman?.notes || ""}
                    ...
                    <div className="flex gap-3 pt-2">
                        ...
                        >
                            {loading ? "جاري الحفظ..." : carRepairman ? "تحديث المصلح" : "إضافة المصلح"}
                        </Button>
                        <Button
                            type="button"
                            onClick={() => router.back()}
