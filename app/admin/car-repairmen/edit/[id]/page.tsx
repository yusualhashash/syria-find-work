...
import CarRepairmanForm from "../../car-repairman-form"

export async function generateStaticParams() {
    return [{ id: "static-placeholder-id" }]
}

export default async function EditCarRepairmanPage({
    params,
}: {
    ...
    const { data: carRepairman, error } = await supabase
        .from("car_repairmen")
        .select("*")
        .eq("id", id)
        .single()
                    ...
                    <div className="flex items-center gap-3 sm:gap-4 mb-6 sm:mb-8">
                        ...
                        <div>
                            ...
                            <p className="text-sm sm:text-base text-gray-600">
                                تحديث بيانات المصلح بشكل كامل
                            </p>
                        </div>
                    </div>

                    <CarRepairmanForm carRepairman={carRepairman} />
                </div>
            </div>
        </div>
    )
