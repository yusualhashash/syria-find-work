...
import { getGermanTeachers, deleteGermanTeacher } from "@/lib/german-teachers/german-teachers-actions"
...
export default function GermanTeachersList() {
    const [germanTeachers, setGermanTeachers] = useState<GermanTeacher[]>([])
    ...
    const loadGermanTeachers = async () => {
        try {
            const data = await getGermanTeachers()
            setGermanTeachers(data)
        } catch (error) {
            console.error("Error loading German teachers:", error)
        } finally {
            setLoading(false)
        }
    }
    ...
    const handleDelete = async (id: string) => {
        try {
            await deleteGermanTeacher(id)
            setGermanTeachers(germanTeachers.filter((t) => t.id !== id))
        } catch (error) {
            console.error("Error deleting German teacher:", error)
    ...
    if (germanTeachers.length === 0) {
        return <div className="text-center py-12 text-gray-600">لا يوجد معلمين ألمان مضافين بعد</div>
    }

    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {germanTeachers.map((teacher) => (
                    ...
                    <CardContent className="p-4 sm:p-6 space-y-4">
                        ...
                        <div className="flex gap-3 pt-2">
                            ...
                            <Button variant="outline" className="flex-1 gap-2 text-sm sm:text-base bg-white text-gray-900 border border-gray-300 hover:bg-gray-100" asChild>
                                <Link href={`/admin/german-teachers/edit/${teacher.id}`}>
