...
import { createGermanTeacher, updateGermanTeacher } from "@/lib/german-teachers/german-teachers-actions"
...
export default function GermanTeacherForm({ germanTeacher }: { germanTeacher?: GermanTeacher }) {
    const router = useRouter()
    const [loading, setLoading] = useState(false)
    const [selectedCity, setSelectedCity] = useState(germanTeacher?.city || "")
    const [selectedGender, setSelectedGender] = useState(germanTeacher?.gender || "")
    const [whatsappNumberValue, setWhatsappNumberValue] = useState<string | undefined>("")
    const [error, setError] = useState<string>("")

    useEffect(() => {
        if (germanTeacher?.whatsapp_number) {
            try {
                const phone = parsePhoneNumber(germanTeacher.whatsapp_number)
                setWhatsappNumberValue(phone?.number)
            } catch {
                setWhatsappNumberValue(germanTeacher.whatsapp_number)
            }
        }
    }, [germanTeacher?.whatsapp_number])
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (germanTeacher) {
                await updateGermanTeacher(germanTeacher.id, formData)
            } else {
                await createGermanTeacher(formData)
            }

            router.push("/admin/german-teachers")
            router.refresh()
        } catch (error: any) {
            console.error("Error saving German teacher:", error)
            ...
            <CardContent className="p-2 sm:p-4">
                <form onSubmit={handleSubmit} className="space-y-2 sm:space-y-4">
                    ...
                    <div className="space-y-2">
                        ...
                        <div className={`${baseInputStyles} ${focusWithinStyles}`}>
                            <Input
                                ...
                                name="name"
                                type="text"
                                defaultValue={germanTeacher?.name}
                    ...
                    <div className="space-y-2">
                        ...
                        <div className={`${baseInputStyles} ${focusWithinStyles}`}>
                            <Input
                                ...
                                name="surname"
                                type="text"
                                defaultValue={germanTeacher?.surname}
                    ...
                    <div className="space-y-2">
                        ...
                        <div className={`${baseInputStyles} ${focusWithinStyles}`}>
                            <Input
                                ...
                                min="1"
                                max="99"
                                defaultValue={germanTeacher?.age}
                    ...
                    <div className="space-y-2">
                        ...
                        <div className={`${textareaGroupStyles} ${focusWithinStyles}`}>
                            <InputGroupTextarea
                                id="address"
                                name="address"
                                defaultValue={germanTeacher?.address || ""}
                    ...
                    <div className="space-y-2">
                        ...
                        <div className={`${baseInputStyles} ${focusWithinStyles}`}>
                            <Input
                                ...
                                type="number"
                                min="0"
                                defaultValue={germanTeacher?.experience_years}
                    ...
                    <div className="space-y-2">
                        ...
                        <div className={`${textareaGroupStyles} ${focusWithinStyles}`}>
                            <InputGroupTextarea
                                id="notes"
                                name="notes"
                                defaultValue={germanTeacher?.notes || ""}
                    ...
                    <div className="flex gap-3 pt-2">
                        ...
                        >
                            {loading ? "جاري الحفظ..." : germanTeacher ? "تحديث المعلم" : "إضافة المعلم"}
                        </Button>
                        <Button
                            type="button"
                            onClick={() => router.back()}
