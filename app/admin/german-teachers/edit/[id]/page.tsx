...
import GermanTeacherForm from "../../german-teacher-form"

export async function generateStaticParams() {
    return [{ id: "static-placeholder-id" }]
}

export default async function EditGermanTeacherPage({
    params,
}: {
    ...
    const { data: germanTeacher, error } = await supabase
    ...
    if (error || !germanTeacher) {
        redirect("/admin/german-teachers")
    }
        ...
        <div dir="rtl" className="min-h-screen bg-linear-to-br from-indigo-50 to-purple-100">
            <div className="max-w-4xl mx-auto p-4 sm:p-6">
                <div className="bg-white rounded-2xl shadow-xl p-4 sm:p-8">
                    ...
                    <GermanTeacherForm germanTeacher={germanTeacher} />
                </div>
            </div>
        </div>
    )
