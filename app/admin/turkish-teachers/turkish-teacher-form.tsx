...
import { SYRIAN_CITIES } from "@/lib/utils" // Reverted import
import { useRouter } from "next/navigation"
import type React from "react"
import { useState, useEffect } from "react"
import { createTurkishTeacher, updateTurkishTeacher } from "@/lib/turkish-teachers/turkish-teachers-actions"
import PhoneInput, { isValidPhoneNumber, parsePhoneNumber } from "react-phone-number-input"
import ar from "react-phone-number-input/locale/ar" // Reverted to Arabic locale
import "react-phone-number-input/style.css"
import { Alert, AlertDescription } from "@/components/ui/alert"
import type { TurkishTeacher } from "@/app/turkish-teachers/turkish-teacher-types"

export default function TurkishTeacherForm({ turkishTeacher }: { turkishTeacher?: TurkishTeacher }) {
    const router = useRouter()
    const [loading, setLoading] = useState(false)
    const [selectedCity, setSelectedCity] = useState(turkishTeacher?.city || "")
    const [selectedGender, setSelectedGender] = useState(turkishTeacher?.gender || "")
    const [whatsappNumberValue, setWhatsappNumberValue] = useState<string | undefined>("")
    const [error, setError] = useState<string>("")

    useEffect(() => {
        if (turkishTeacher?.whatsapp_number) {
            try {
                const phone = parsePhoneNumber(turkishTeacher.whatsapp_number)
                setWhatsappNumberValue(phone?.number)
            } catch {
                setWhatsappNumberValue(turkishTeacher.whatsapp_number)
            }
        }
    }, [turkishTeacher?.whatsapp_number])
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (turkishTeacher) {
                await updateTurkishTeacher(turkishTeacher.id, formData)
            } else {
                await createTurkishTeacher(formData)
            }

            router.push("/admin/turkish-teachers")
            router.refresh()
        } catch (error: any) {
            console.error("Error saving Turkish teacher:", error)
            ...
            <CardContent className="p-2 sm:p-4">
                <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
                    ...
                    <div className="space-y-2">
                        ...
                        <div className={`${baseInputStyles} ${focusWithinStyles}`}>
                            <Input
                                ...
                                name="name"
                                type="text"
                                defaultValue={turkishTeacher?.name}
                    ...
                    <div className="space-y-2">
                        ...
                        <div className={`${baseInputStyles} ${focusWithinStyles}`}>
                            <Input
                                ...
                                name="surname"
                                type="text"
                                defaultValue={turkishTeacher?.surname}
                    ...
                    <div className="space-y-2">
                        ...
                        <div className={`${baseInputStyles} ${focusWithinStyles}`}>
                            <Input
                                ...
                                min="1"
                                max="99"
                                defaultValue={turkishTeacher?.age}
                    ...
                    <div className="space-y-2">
                        ...
                        <div className={`${textareaGroupStyles} ${focusWithinStyles}`}>
                            <InputGroupTextarea
                                id="address"
                                name="address"
                                defaultValue={turkishTeacher?.address || ""}
                    ...
                    <div className="space-y-2">
                        ...
                        <div className={`${baseInputStyles} ${focusWithinStyles}`}>
                            <Input
                                ...
                                type="number"
                                min="0"
                                defaultValue={turkishTeacher?.experience_years}
                    ...
                    <div className="space-y-2">
                        ...
                        <div className={`${textareaGroupStyles} ${focusWithinStyles}`}>
                            <InputGroupTextarea
                                id="notes"
                                name="notes"
                                defaultValue={turkishTeacher?.notes || ""}
                    ...
                    <div className="flex gap-3 pt-2">
                        ...
                        >
                            {loading ? "جاري الحفظ..." : turkishTeacher ? "تحديث المدرس" : "إضافة المدرس"}
                        </Button>
                        <Button
                            type="button"
                            onClick={() => router.back()}
