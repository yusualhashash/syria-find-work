...
interface TurkishTeacherDetailProps {
    turkishTeacher: TurkishTeacher
    isFavorited: boolean
}

export default function TurkishTeacherDetail({ turkishTeacher, isFavorited }: TurkishTeacherDetailProps) {
    const phoneNumber = turkishTeacher.whatsapp_number.replace(/\D/g, "")
    const whatsappLink = `https://wa.me/${phoneNumber}`
    const message = encodeURIComponent(`مرحباً، أحتاج للاستفسار عن ${turkishTeacher.name} ${turkishTeacher.surname}`)
    const whatsappLinkWithMessage = `${whatsappLink}?text=${message}`

    const teacher = turkishTeacher
        ...
        <div dir="rtl" className="min-h-screen bg-[#1a1a1a]">
            <div className="p-4 space-y-4">
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        <Link
                            ...
                            className="p-0 hover:bg-gray-800 rounded-lg transition-colors"
                        >
                            <ArrowRight className="h-6 w-6 text-white" />
                        </Link>
                        <h1 className="text-xl font-bold text-white">تفاصيل المدرس</h1>
                    </div>

                    <FavoriteTurkishTeacherButton
                ...
                <Card className="p-6 bg-[#2a2a2a] border-gray-700">
                    <div className="flex flex-col items-center text-center mb-6">
                        ...
                        <p className="text-gray-400">
                            {teacher.gender === "male" ? "مدرس لغة تركية" : "مدرسة لغة تركية"}
                        </p>
                    </div>
                ...
                <a
                    ...
                    className="flex items-center justify-center gap-3 bg-linear-to-r from-green-500 to-green-600 text-white py-4 px-6 rounded-xl font-bold text-lg hover:from-blue-600 hover:to-blue-700 transition-all shadow-lg"
                >
