...
export default function CarRepairmenClient({ initialRepairmen }: { initialRepairmen: CarRepairmanWithFavorite[] }) {
    const [searchTerm, setSearchTerm] = useState("")
    const [selectedCity, setSelectedCity] = useState<string | null>(null)
    const [allRepairmen, setAllRepairmen] = useState<CarRepairmanWithFavorite[]>(initialRepairmen)

    const filteredRepairmen = useMemo(() => {
        return allRepairmen.filter((repairman) => {
            const matchesSearchTerm = repairman.name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                repairman.surname?.toLowerCase().includes(searchTerm.toLowerCase())
            const matchesCity = selectedCity ? repairman.city === selectedCity : true
            return matchesSearchTerm && matchesCity
        })
    }, [allRepairmen, searchTerm, selectedCity])
            ...
            <div className="max-w-7xl mx-auto p-4 md:p-6">
                    ...
                    <div className="flex items-center justify-between py-3">
                        <div className="flex items-center gap-4">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <h1 className="text-xl md:text-2xl font-bold text-white"> مصلحين سيارات </h1>
                            </div>
                        </div>
                    </div>
                ...
                {filteredRepairmen.length === 0 ? (
                    <div className="text-center py-12 pt-4">
                        ...
                        <p className="text-gray-400 text-lg">لا يوجد مصلحين سيارات متاحين حالياً</p>
                    </div>
                ) : (
                    <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-6 pt-4">
                        {filteredRepairmen.map((repairman) => (
                            <Link key={repairman.id} href={`/car-repairmen/${repairman.id}`} className="h-full">
                                <div className="bg-gray-900 border border-gray-700 rounded-none overflow-hidden transition-shadow h-full flex flex-col relative">
                                    <div className="absolute top-2 left-2 z-10">
                                        <FavoriteCarRepairmanButton
                                            repairmanId={repairman.id}
                                            initialIsFavorited={repairman.isFavorited}
                                            size="sm"
                                        />
                                    </div>
                                    ...
                                    <div className="p-2 md:p-6 grow flex flex-col">
                                        <div className="mb-2">
                                            <img
                                                ...
                                                alt={repairman.gender === "male" ? "Male Repairman" : "Female Repairman"}
                                                className="w-full h-36 object-cover bg-gray-800 mb-2"
                                            />
                                            <h3 className="text-base md:text-lg font-bold text-white text-center">{repairman.name} {repairman.surname}</h3>
                                        </div>
                                        <div className="space-y-1 md:space-y-2 mt-auto">
                                            {repairman.city && (
                                                <div className="flex items-center gap-2 text-gray-400">
                                                    <MapPin className="w-3 h-3 md:w-4 md:h-4" />
                                                    <span className="text-xs md:text-sm">{repairman.city}</span>
                                                </div>
                                            )}
                                            {repairman.address && (
                                                <div className="flex items-center gap-2 text-gray-400">
                                                    <Home className="w-3 h-3 md:w-4 md:h-4" />
                                                    <span className="text-xs md:text-sm">{repairman.address}</span>
                                                </div>
                                            )}
                                            {repairman.age && (
                                                <div className="flex items-center gap-2 text-gray-400">
                                                    <Cake className="w-3 h-3 md:w-4 md:h-4" />
                                                    <span className="text-xs md:text-sm">{repairman.age} سنة</span>
                                                </div>
                                            )}
                                            {repairman.whatsapp_number && (
                                                <div className="flex items-center gap-2 text-gray-400">
                                                    <Phone className="w-3 h-3 md:w-4 md:h-4" />
                                                    <span className="text-xs md:text-sm">{repairman.whatsapp_number}</span>
                                                </div>
                                            )}
                                            {repairman.experience_years && (
                                                <div className="flex items-center gap-2 text-gray-400">
                                                    <BookOpenText className="w-3 h-3 md:w-4 md:h-4" />
                                                    <span className="text-xs md:text-sm">خبرة: {repairman.experience_years} سنوات</span>
                                                </div>
                                            )}
                                        </div>
                                    </div>
