...
interface EnglishTeacherDetailProps {
    englishTeacher: EnglishTeacher
    isFavorited: boolean
}

export default function EnglishTeacherDetail({ englishTeacher, isFavorited }: EnglishTeacherDetailProps) {
    const phoneNumber = englishTeacher.whatsapp_number.replace(/\D/g, "")
    const whatsappLink = `https://wa.me/${phoneNumber}`
    const message = encodeURIComponent(`مرحباً، أحتاج للاستفسار عن ${englishTeacher.name} ${englishTeacher.surname}`)
    const whatsappLinkWithMessage = `${whatsappLink}?text=${message}`

    const teacher = englishTeacher
            ...
            <div className="p-4 space-y-4">
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        <Link href="/english-teachers" className="p-0 hover:bg-gray-800 rounded-lg transition-colors">
                ...
                <Card className="p-6 bg-[#2a2a2a] border-gray-700">
                    <div className="flex flex-col items-center text-center mb-6">
                        ...
                        <p className="text-gray-400">
                            {teacher.gender === "male" ? "مدرس لغة إنجليزية" : "مدرسة لغة إنجليزية"}
                        </p>
