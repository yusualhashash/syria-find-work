...
export default function GermanTeachersClient({ initialTeachers }: { initialTeachers: GermanTeacherWithFavorite[] }) {
    const [searchTerm, setSearchTerm] = useState("")
    const [selectedCity, setSelectedCity] = useState<string | null>(null)
    const [allTeachers, setAllTeachers] = useState<GermanTeacherWithFavorite[]>(initialTeachers)
            ...
            <div className="max-w-7xl mx-auto p-4 md:p-6">
                    ...
                    <div className="flex items-center justify-between py-3">
                        <div className="flex items-center gap-4">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <h1 className="text-xl md:text-2xl font-bold text-white"> معلمين ألمان </h1>
                            </div>
                        </div>
                    </div>
                ...
                {filteredTeachers.length === 0 ? (
                    <div className="text-center py-12 pt-4">
                        ...
                        <p className="text-gray-400 text-lg">لا يوجد معلمين ألمان متاحين حالياً</p>
                    </div>
                ) : (
                    <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-6 pt-4">
                        {filteredTeachers.map((teacher) => (
                            <Link key={teacher.id} href={`/german-teachers/${teacher.id}`} className="h-full">
