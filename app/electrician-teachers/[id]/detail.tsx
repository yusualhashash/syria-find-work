...
interface ElectricianTeacherDetailProps {
    electricianTeacher: ElectricianTeacher
    isFavorited: boolean
}

export default function ElectricianTeacherDetail({ electricianTeacher, isFavorited }: ElectricianTeacherDetailProps) {
    const phoneNumber = electricianTeacher.whatsapp_number.replace(/\D/g, "")
    const whatsappLink = `https://wa.me/${phoneNumber}`
    const message = encodeURIComponent(`مرحباً، أحتاج للاستفسار عن ${electricianTeacher.name} ${electricianTeacher.surname}`)
    const whatsappLinkWithMessage = `${whatsappLink}?text=${message}`

    const teacher = electricianTeacher
            ...
            <div className="p-4 space-y-4">
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        <Link href="/electrician-teachers" className="p-0 hover:bg-gray-800 rounded-lg transition-colors">
                            <ArrowRight className="h-6 w-6 text-white" />
                        </Link>
                        <h1 className="text-xl font-bold text-white">تفاصيل المعلم</h1>
                    </div>
                    <div className="flex items-center gap-2">
                        <FavoriteElectricianTeacherButton teacherId={teacher.id} initialIsFavorited={isFavorited} size="md" />
                    </div>
                </div>
                ...
                <Card className="p-6 bg-[#2a2a2a] border-gray-700">
                    <div className="flex flex-col items-center text-center mb-6">
                        ...
                        <p className="text-gray-400">معلم كهرباء</p>
                    </div>
