...
export default function PlumbersClient({ initialPlumbers }: { initialPlumbers: PlumberWithFavorite[] }) {
    const [searchTerm, setSearchTerm] = useState("")
    const [selectedCity, setSelectedCity] = useState<string | null>(null)
    const [allPlumbers, setAllPlumbers] = useState<PlumberWithFavorite[]>(initialPlumbers)

    const filteredPlumbers = useMemo(() => {
        return allPlumbers.filter((plumber) => {
            const matchesSearchTerm = plumber.name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                plumber.surname?.toLowerCase().includes(searchTerm.toLowerCase())
            const matchesCity = selectedCity ? plumber.city === selectedCity : true
            return matchesSearchTerm && matchesCity
        })
    }, [allPlumbers, searchTerm, selectedCity])
            ...
            <div className="max-w-7xl mx-auto p-4 md:p-6">
                    ...
                    <div className="flex items-center justify-between py-3">
                        <div className="flex items-center gap-4">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <h1 className="text-xl md:text-2xl font-bold text-white"> معلمين تمديدات صحية </h1>
                            </div>
                        </div>
                    </div>
                ...
                {filteredPlumbers.length === 0 ? (
                    <div className="text-center py-12 pt-4">
                        ...
                        <p className="text-gray-400 text-lg">لا يوجد معلمين تمديدات صحية متاحين حالياً</p>
                    </div>
                ) : (
                    <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-6 pt-4">
                        {filteredPlumbers.map((plumber) => (
                            <Link key={plumber.id} href={`/plumbers/${plumber.id}`} className="h-full">
                                <div className="bg-gray-900 border border-gray-700 rounded-none overflow-hidden transition-shadow h-full flex flex-col relative">
                                    <div className="absolute top-2 left-2 z-10">
                                        <FavoritePlumberButton
                                            plumberId={plumber.id}
                                            initialIsFavorited={plumber.isFavorited}
                                            size="sm"
                                        />
                                    </div>
                                    ...
                                    <div className="p-2 md:p-6 grow flex flex-col">
                                        <div className="mb-2">
                                            <img
                                                src={plumber.gender === "male" ? "/men.png" : "/woman.png"}
                                                alt={plumber.gender === "male" ? "Male Plumber" : "Female Plumber"}
                                                className="w-full h-36 object-cover bg-gray-800 mb-2"
                                            />
                                            <h3 className="text-base md:text-lg font-bold text-white text-center">{plumber.name} {plumber.surname}</h3>
                                        </div>
                                        <div className="space-y-1 md:space-y-2 mt-auto">
                                            {plumber.city && (
                                                <div className="flex items-center gap-2 text-gray-400">
                                                    <MapPin className="w-3 h-3 md:w-4 md:h-4" />
                                                    <span className="text-xs md:text-sm">{plumber.city}</span>
                                                </div>
                                            )}
                                            {plumber.address && (
                                                <div className="flex items-center gap-2 text-gray-400">
                                                    <Home className="w-3 h-3 md:w-4 md:h-4" />
                                                    <span className="text-xs md:text-sm">{plumber.address}</span>
                                                </div>
                                            )}
                                            {plumber.age && (
                                                <div className="flex items-center gap-2 text-gray-400">
                                                    <Cake className="w-3 h-3 md:w-4 md:h-4" />
                                                    <span className="text-xs md:text-sm">{plumber.age} سنة</span>
                                                </div>
                                            )}
                                            {plumber.whatsapp_number && (
                                                <div className="flex items-center gap-2 text-gray-400">
                                                    <Phone className="w-3 h-3 md:w-4 md:h-4" />
                                                    <span className="text-xs md:text-sm">{plumber.whatsapp_number}</span>
                                                </div>
                                            )}
                                            {plumber.experience_years && (
                                                <div className="flex items-center gap-2 text-gray-400">
                                                    <BookOpenText className="w-3 h-3 md:w-4 md:h-4" />
                                                    <span className="text-xs md:text-sm">خبرة: {plumber.experience_years} سنوات</span>
                                                </div>
                                            )}
                                        </div>
                                    </div>
